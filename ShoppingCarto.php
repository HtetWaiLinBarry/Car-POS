...
session_start();
  ...
  {
    $CartID=AutoID('Cart','OrderID','Or_',4);
    $CartDate=date('Y-m-d');
    $CustomerID=$_SESSION['CustomerID'];
    $PurchaseAmount=calculateTotal();
    $taxamount=calculateTax();
    $netamount=netAmount();
    $paymenttype=$_SESSION['rdopaymenttype'];
    $insert="INSERT into Cart C
            values('$CartID','$CartDate','$CutomerID','$PurchaseAmount','$taxamount','$netamount')";
     ...
     for($i=0;$i<$count;$i++)
      {
         ...
         $insert="INSERT into CartDetail values('$CartID','$vid','$price','$quantity')";
...
<body>
   <form action="ShoppingCarto.php" method="post">
     <fieldset>
